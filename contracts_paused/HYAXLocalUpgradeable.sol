 ...
 */
contract HYAXLocalUpgradeable is
    ...
    ReentrancyGuardUpgradeable
{
     ...
     */
    event InvestorAddedToQualifiedInvestorList(
        address sender,
     ...
     */
    event InvestorRemovedFromQualifiedInvestorList(
        address sender,
        address _QualifiedInvestorAddress
    );
        ...
        returns (uint256 totalInvestmentInUsd, uint256 totalHyaxTokenToReturn)
    {
        ...
        totalHyaxTokenToReturn =
            (totalInvestmentInUsd / hyaxPrice) *
            decimalsInMathOperation;
    ...
    function addToWhiteList(
        ...
        require(
            ...
            "Investor address to add to the white list can not be the zero address"
        );

        // Ensure that the investor address has not already been added to the white list
        require(
            investorsWhiteList[_investorAddress] != true,
            "That investor address has already been added to the white list"
        );
    ...
    function removeFromWhiteList(
        ...
        require(
            _investorAddress != address(0),
            "Investor address to remove from the white list can not be the zero address"
        );

        // Ensure that the investor address is registered on the white list
        require(
            investorsWhiteList[_investorAddress] == true,
            "That investor address is not registered on the white list"
        );
    ...
    modifier investorIsOnWhiteList() {
        ...
        require(
            investorsWhiteList[msg.sender] == true,
    ...
    modifier onlyOwnerOrWhitelister() {
        ...
        require(
            ...
            "Function reserved only for the white lister address or the owner"
        );
        _;
    }
     ...
     */
    function addToQualifiedInvestorList(
        address _qualifiedInvestorAddress
    ) external onlyOwnerOrWhitelister {
        // Ensure that the investor address to add is not the zero address
        require(
            _qualifiedInvestorAddress != address(0),
            "Investor address to add to the qualified investor list can not be the zero address"
        );

        // Ensure that the investor address to add is already in the white list of investors
        require(
            investorsWhiteList[_qualifiedInvestorAddress] == true,
            "Investor address must be first added to the investor white list"
        );
        ...
        require(
            ...
            "That investor address has already been added to the qualified investor list"
        );
        ...
        emit InvestorAddedToQualifiedInvestorList(
            msg.sender,
     ...
     */
    function removeFromQualifiedInvestorList(
        address _qualifiedInvestorAddress
    ) external onlyOwnerOrWhitelister {
        // Ensure that the investor address to remove is not the zero address
        require(
            _qualifiedInvestorAddress != address(0),
            "Investor address to remove from the qualified investor list can not be the zero address"
        );

        // Ensure that the investor address is registered on the qualified investor list
        require(
            qualifiedInvestorList[_qualifiedInvestorAddress] == true,
            "That investor address is not registered on the qualified investor list"
        );
        ...
        emit InvestorRemovedFromQualifiedInvestorList(
            msg.sender,
            _qualifiedInvestorAddress
        );
    }
    ...
    function tokenIssuance(uint256 _amount) public onlyOwner nonReentrant {
        ...
        require(
            newTotalSupply <= 10000000000 * 10 ** decimals(),
     ...
     */
    function validateMaximumInvestedAmountAndInvestorLimit(
        ...
        uint256 newTotalAmountInvestedInUSD = _totalInvestmentInUsd +
            totalUsdDepositedByInvestor[_investorAddress];
        ...
        if (newTotalAmountInvestedInUSD > maximumInvestmentAllowedInUSD) {
            require(
                qualifiedInvestorList[_investorAddress] == true,
        ...
        returns (bool)
    {
        ...
        bool successSendingHyaxToken = this.transfer(
            msg.sender,
            totalHyaxTokenToReturn
        );
        require(
            successSendingHyaxToken,
     ...
     * @return A boolean indicating the success of the investment and HYAX token transfer.
     */
    function investFromUsdc(
        uint256 _amount
    ) external investorIsOnWhiteList nonReentrant returns (bool) {
        //Calculate total HYAX to return while validating minimum investment and if there are HYAX tokens left to sell
        (uint256 totalInvestmentInUsd, uint256 totalHyaxTokenToReturn) = this
            .calculateTotalHyaxTokenToReturn(
                _amount,
                this.getCurrentUsdcPrice()
            );
        ...
        bool successReceivingUsdc = usdcToken.transferFrom(
            msg.sender,
            address(this),
            _amount
        );
        require(
            successReceivingUsdc,
            "There was an error on receiving the USDC investment"
        );

        //Transfer USDC to the treasury address
        bool successSendingUsdc = usdcToken.transfer(
            payable(treasuryAddress),
            _amount
        );
        require(
            successSendingUsdc,
            "There was an error on sending the USDC investment to the treasury"
        );
    ...
    function investFromUsdt(
        ...
        bool successReceivingUsdt = usdtToken.transferFrom(
            msg.sender,
            address(this),
            _amount
        );
        require(
            successReceivingUsdt,
            "There was an error on receiving the USDT investment"
        );

        //Transfer USDT to the treasury address
        bool successSendingUsdt = usdtToken.transfer(
            payable(treasuryAddress),
            _amount
        );
        require(
            successSendingUsdt,
            "There was an error on sending the USDT investment to the treasury"
        );
    ...
    function investFromWbtc(
        ...
        bool successReceivingWbtc = wbtcToken.transferFrom(
            msg.sender,
            address(this),
            _amount
        );
        require(
            successReceivingWbtc,
            "There was an error on receiving the WBTC investment"
        );

        //Transfer WBTC to the treasury address
        bool successSendingWbtc = wbtcToken.transfer(
            payable(treasuryAddress),
            _amount
        );
        require(
            successSendingWbtc,
            "There was an error on sending the WBTC investment to the treasury"
        );
    ...
    function investFromWeth(
        ...
        bool successReceivingWeth = wethToken.transferFrom(
            msg.sender,
            address(this),
            _amount
        );
        require(
            successReceivingWeth,
            "There was an error on receiving the WETH investment"
        );

        //Transfer WETH to the treasury address
        bool successSendingWeth = wethToken.transfer(
            payable(treasuryAddress),
            _amount
        );
        require(
            successSendingWeth,
            "There was an error on sending the WETH investment to the treasury"
        );
    ...
    function updateWhiteListerAddress(
        ...
        require(
            ...
            "The white lister address can not be the zero address"
        );

        //Ensure that the update transaction is not repeated for the same parameter, just as a good practice
        require(
            _newWhiteListerAddress != whiteListerAddress,
            "White lister address has already been modified to that value"
        );
    ...
    function updateTreasuryAddress(
        ...
        require(
            ...
            "The treasury address can not be the zero address"
        );
     ...
     */
    function updateMaticPriceFeedAddress(
        address _newMaticPriceFeedAddress
    ) external onlyOwner {
        // Ensure the new MATIC price feed address is not the zero address
        require(
            _newMaticPriceFeedAddress != address(0),
            "The price data feed address can not be the zero address"
        );

        //Ensure that the update transaction is not repeated for the same parameter, just as a good practice
        require(
            _newMaticPriceFeedAddress != maticPriceFeedAddress,
        ...
        try tempDataFeedMatic.description() returns (
            ...
            require(
                hashOfExpectedMaticFeedDescription ==
                    hashOfCurrentMaticFeedDescription,
    ...
    function updateUsdcTokenAddress(
        address _newUsdcTokenAddress
    ) external onlyOwner {
        // Ensure the new USDC token address is not the zero address
        require(
            _newUsdcTokenAddress != address(0),
            "The token address can not be the zero address"
        );

        //Ensure that the update transaction is not repeated for the same parameter, just as a good practice
        require(
            _newUsdcTokenAddress != usdcTokenAddress,
     ...
     */
    function updateUsdcPriceFeedAddress(
        address _newUsdcPriceFeedAddress
    ) external onlyOwner {
        // Ensure the new USDC price feed address is not the zero address
        require(
            _newUsdcPriceFeedAddress != address(0),
            "The price data feed address can not be the zero address"
        );

        //Ensure that the update transaction is not repeated for the same parameter, just as a good practice
        require(
            _newUsdcPriceFeedAddress != usdcPriceFeedAddress,
            "USDC price feed address has already been modified to that value"
        );

        //Temporary data feed to perform the validation of the data feed descriptions
        AggregatorV3Interface tempDataFeedUsdc = AggregatorV3Interface(
            _newUsdcPriceFeedAddress
        );

        //Validate if the new address is actually a price feed address. Attempt to call the description function
        try tempDataFeedUsdc.description() returns (
            string memory descriptionValue
        ) {
            //Get the hash value of the USDC/USD string
            bytes32 hashOfExpectedUsdcFeedDescription = keccak256(
                abi.encodePacked("USDC / USD")
            );

            //Get the hash value of the description of the price data feed
            bytes32 hashOfCurrentUsdcFeedDescription = keccak256(
                abi.encodePacked(descriptionValue)
            );
            ...
            require(
                hashOfExpectedUsdcFeedDescription ==
                    hashOfCurrentUsdcFeedDescription,
                "The new address does not seem to belong to a USDC price data feed"
            );
        } catch {
            //In case there is an error obtaining the description of the data feed, revert the transaction
            revert(
    ...
    function updateUsdtTokenAddress(
        ...
        require(
            _newUsdtTokenAddress != address(0),
            "The token address can not be the zero address"
        );

        //Ensure that the update transaction is not repeated for the same parameter, just as a good practice
        require(
            _newUsdtTokenAddress != usdtTokenAddress,
     ...
     */
    function updateUsdtPriceFeedAddress(
        address _newUsdtPriceFeedAddress
    ) external onlyOwner {
        // Ensure the new USDT price feed address is not the zero address
        require(
            _newUsdtPriceFeedAddress != address(0),
            "The price data feed address can not be the zero address"
        );

        //Ensure that the update transaction is not repeated for the same parameter, just as a good practice
        require(
            _newUsdtPriceFeedAddress != usdtPriceFeedAddress,
            "USDT price feed address has already been modified to that value"
        );

        //Temporary data feed to perform the validation of the data feed descriptions
        AggregatorV3Interface tempDataFeedUsdt = AggregatorV3Interface(
            _newUsdtPriceFeedAddress
        );

        //Validate if the new address is actually a price feed address. Attempt to call the description function
        try tempDataFeedUsdt.description() returns (
            string memory descriptionValue
        ) {
            //Get the hash value of the USDT/USD string
            bytes32 hashOfExpectedUsdtFeedDescription = keccak256(
                abi.encodePacked("USDT / USD")
            );

            //Get the hash value of the description of the price data feed
            bytes32 hashOfCurrentUsdtFeedDescription = keccak256(
                abi.encodePacked(descriptionValue)
            );
            ...
            require(
                hashOfExpectedUsdtFeedDescription ==
                    hashOfCurrentUsdtFeedDescription,
                "The new address does not seem to belong to a USDT price data feed"
            );
        } catch {
            //In case there is an error obtaining the description of the data feed, revert the transaction
            revert(
     ...
     */
    function updateWbtcTokenAddress(
        address _newWbtcTokenAddress
    ) external onlyOwner {
        // Ensure the new WBTC token address is not the zero address
        require(
            _newWbtcTokenAddress != address(0),
            "The token address can not be the zero address"
        );

        //Ensure that the update transaction is not repeated for the same parameter, just as a good practice
        require(
            _newWbtcTokenAddress != wbtcTokenAddress,
     ...
     */
    function updateWbtcPriceFeedAddress(
        address _newWbtcPriceFeedAddress
    ) external onlyOwner {
        // Ensure the new WBTC price feed address is not the zero address
        require(
            _newWbtcPriceFeedAddress != address(0),
            "The price data feed address can not be the zero address"
        );

        //Ensure that the update transaction is not repeated for the same parameter, just as a good practice
        require(
            _newWbtcPriceFeedAddress != wbtcPriceFeedAddress,
            "WBTC price feed address has already been modified to that value"
        );

        //Temporary data feed to perform the validation of the data feed descriptions
        AggregatorV3Interface tempDataFeedWbtc = AggregatorV3Interface(
            _newWbtcPriceFeedAddress
        );

        //Validate if the new address is actually a price feed address. Attempt to call the description function
        try tempDataFeedWbtc.description() returns (
            string memory descriptionValue
        ) {
            //Get the hash value of the WBTC/USD string
            bytes32 hashOfExpectedWbtcFeedDescription = keccak256(
                abi.encodePacked("WBTC / USD")
            );

            //Get the hash value of the description of the price data feed
            bytes32 hashOfCurrentWbtcFeedDescription = keccak256(
                abi.encodePacked(descriptionValue)
            );
            ...
            require(
                hashOfExpectedWbtcFeedDescription ==
                    hashOfCurrentWbtcFeedDescription,
                "The new address does not seem to belong to a WBTC price data feed"
            );
        } catch {
            //In case there is an error obtaining the description of the data feed, revert the transaction
            revert(
     ...
     */
    function updateWethTokenAddress(
        address _newWethTokenAddress
    ) external onlyOwner {
        // Ensure the new WETH token address is not the zero address
        require(
            _newWethTokenAddress != address(0),
            "The token address can not be the zero address"
        );

        //Ensure that the update transaction is not repeated for the same parameter, just as a good practice
        require(
            _newWethTokenAddress != wethTokenAddress,
     ...
     */
    function updateWethPriceFeedAddress(
        address _newWethPriceFeedAddress
    ) external onlyOwner {
        // Ensure the new WETH price feed address is not the zero address
        require(
            _newWethPriceFeedAddress != address(0),
            "The price data feed address can not be the zero address"
        );

        //Ensure that the update transaction is not repeated for the same parameter, just as a good practice
        require(
            _newWethPriceFeedAddress != wethPriceFeedAddress,
            "WETH price feed address has already been modified to that value"
        );

        //Temporary data feed to perform the validation of the data feed descriptions
        AggregatorV3Interface tempDataFeedWeth = AggregatorV3Interface(
            _newWethPriceFeedAddress
        );

        //Validate if the new address is actually a price feed address. Attempt to call the description function
        try tempDataFeedWeth.description() returns (
            string memory descriptionValue
        ) {
            //Get the hash value of the ETH/USD string
            bytes32 hashOfExpectedWethFeedDescription = keccak256(
                abi.encodePacked("ETH / USD")
            );

            //Get the hash value of the description of the price data feed
            bytes32 hashOfCurrentWethFeedDescription = keccak256(
                abi.encodePacked(descriptionValue)
            );
            ...
            require(
                hashOfExpectedWethFeedDescription ==
                    hashOfCurrentWethFeedDescription,
                "The new address does not seem to belong to a ETH price data feed"
            );
        } catch {
            //In case there is an error obtaining the description of the data feed, revert the transaction
            revert(
    ...
    function getCurrentMaticPrice() public view returns (uint256) {
        try dataFeedMatic.latestRoundData() returns (
            ...
            revert(
                "There was an error obtaining the MATIC price from the oracle"
            );
        }
    }
    ...
    function getCurrentUsdcPrice() public view returns (uint256) {
        try dataFeedUsdc.latestRoundData() returns (
            ...
            revert(
                "There was an error obtaining the USDC price from the oracle"
            );
        }
    }
    ...
    function getCurrentUsdtPrice() public view returns (uint256) {
        try dataFeedUsdt.latestRoundData() returns (
            ...
            revert(
                "There was an error obtaining the USDT price from the oracle"
            );
        }
    }
    ...
    function getCurrentWbtcPrice() public view returns (uint256) {
        try dataFeedWbtc.latestRoundData() returns (
            ...
            revert(
                "There was an error obtaining the WBTC price from the oracle"
            );
        }
    }
    ...
    function getCurrentWethPrice() public view returns (uint256) {
        try dataFeedWeth.latestRoundData() returns (
            ...
            revert(
                "There was an error obtaining the WETH price from the oracle"
            );
        }
    }
    ...
    function transferOwnership(
        ...
        require(
            ...
            "Ownable: new owner can not be the same contract address"
        );
